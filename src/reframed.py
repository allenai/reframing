...
def generate_responses(args):
    ...
    for task in list_task[start:end]:
        ...
        with open('output_files_reframed/' + args.model_name + "/" +task+'_prediction_new.json', 'r', encoding='utf-8') as f:
                true_answers = json.load(f)["true"]
                
        with open('output_files_reframed/' + args.model_name + "/" +task+'_prediction_new.json', 'w', encoding='utf-8') as f:
            f.write(json.dumps({"true": true_answers, "prediction": task_answers}, ensure_ascii=False, indent = 4))
