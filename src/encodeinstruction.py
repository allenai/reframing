...
def encodeinstruction (task, model_name, instruction_structure =['Definition','Prompt','Things to Avoid','Emphasis & Caution', 'Negative Examples Full Explanations', 'Positive Examples Full Explanations'], number_of_examples=0, number_of_instances= 100):
    ...
    for i in range(number_of_instances):
        ...
        true_answers.append(data["Instances"][indexlist[i]]["output"])
        true_answers_new.append(' '.join(data["Instances"][indexlist[i]]["output"]))
    task_answers = []
    with open('output_files/' + model_name + "/" +task+'_prediction.json', 'w', encoding='utf-8') as f:
        f.write(json.dumps({"true": true_answers, "prediction": task_answers}, ensure_ascii=False, indent = 4))
        
    with open('output_files/' + model_name + "/" +task+'_prediction_new.json', 'w', encoding='utf-8') as f:
